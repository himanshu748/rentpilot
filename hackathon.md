...
RentPilot searches for rooms using a renter's city, preferred areas, budget, home type and must-haves. It separates web leads from matches backed by permitted listing evidence. OpenAI drafts inquiries for the renter to review before AgentMail sends them. The full inquiry-and-reply loop is still under verification; the status below distinguishes working paths from unfinished tests.
...
OpenAI inference uses Vercel AI Gateway with `AI_GATEWAY_API_KEY` stored only on Convex. The model is pinned to `openai/gpt-4o-mini` and the provider allowlist to `openai`, with no fallback. A live gateway probe returned HTTP 200 and a valid structured rental draft on September 5, 2026. The signed-in production draft/approval/send loop still needs to be exercised before submission. Vercel supplies inference access only; it does not host the app.
...
The production sender inbox and least-privilege API key are configured. Registration of the production `message.received` webhook is still pending because the inbox-scoped key cannot manage webhooks. Outbound sign-in and inquiry flows can be demonstrated without that webhook; the webhook must be registered before demonstrating inbound replies.
...
- [ ] Record a real model-authored inquiry through the signed-in product flow
- [ ] Register AgentMail `message.received` webhook and verify a reply round trip
