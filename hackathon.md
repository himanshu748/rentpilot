...
- `npm test` — 75 tests pass, including offered-unit extraction, reply ingestion, sign-in recovery, filtered detail selection, progressive search updates, search ownership and concurrent-run protection. External services are stubbed in the automated suite.
...
- Authenticated production notebook check, September 12: normal email-code sign-in succeeded through a controlled inbox. In the public app, the tester saved an existing Roomies link from that account's live-search results, added explicitly fictional QA notes, and changed the manual stage to Not suitable. A full reload preserved the notes, stage, September 5 source date, original requirements, and unverified status. The tester removed only that QA bookmark, reloaded to confirm the original empty notebook, and signed out. Browser errors and warnings were zero. The check used normal UI actions against production Convex; it did not inject fixture data or run a new search.
- The published video still records the September 5 build. The September 12 check proves hosted notebook persistence, separately from the September 5 Firecrawl → OpenAI → approved AgentMail inquiry → reply test. No new rental source or vacancy was verified, and no landlord was contacted. Private QA screenshots and receipts remain ignored locally.
...
September 7 discovery update: searches now combine area/budget discovery with an amenities query without relaxing the eligibility rules. Convex persists searching, checking, completion and failure states; source links appear before permitted extraction finishes. Leads have filters, a requirement checklist and copyable questions for manual contact. UI fixture checks cover 1280px, 375px and 320px, copying, filters, retries and reduced motion. These fixtures are not inventory. After deployment, a fresh anonymous browser saved the Bithauli INR8,000 brief, rendered the new panel, reached the sign-in gate, and retained the brief on reload without runtime errors; the 375px layout had no horizontal overflow. Production still has only the sample source and demo feed approved; no real Bithauli vacancy with all requested amenities has been established. The updated authenticated provider search has not yet been exercised end to end.
