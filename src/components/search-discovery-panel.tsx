...
import { ArrowUpRight, Check, Copy, LoaderCircle, Search } from "lucide-react";
...
type Run = { query: string; queries?: string[]; results: Lead[]; searchedAt: number; phase?: "searching" | "checking" | "complete" | "failed"; error?: string };
...
function LeadRow({ lead, brief }: { lead: Lead; brief: Brief }) {
  ...
  return <li className="discovery-lead">
    ...
    <div className="discovery-lead-actions"><a href={lead.url} target="_blank" rel="noopener noreferrer">Open source <ArrowUpRight size={14} aria-hidden="true" /><span className="sr-only"> (new tab)</span></a><button type="button" onClick={copy}>{copied ? <Check size={14} aria-hidden="true" /> : <Copy size={14} aria-hidden="true" />}<span aria-live="polite">{copied ? "Questions copied" : "Copy questions"}</span></button></div>
...
export function SearchDiscoveryPanel({ brief, run, searching, error, onSearch, onEdit }: { brief: Brief; run: Run | null | undefined; searching: boolean; error: string | null; onSearch: () => void; onEdit: () => void }) {
  ...
  const phase = busy ? run?.phase === "checking" ? "Checking permitted pages" : "Finding source links" : failed ? "Search interrupted" : run ? "Search complete" : "Ready to search";
  return <section className="discovery-panel" aria-labelledby="discovery-title">
    ...
    {leads.length > 0 && <>
      ...
      <ul className="discovery-results">{visible.map(lead => <LeadRow key={lead.url} lead={lead} brief={brief} />)}</ul>
    ...
    {run && <details className="discovery-query"><summary>Search details</summary><p>Last update: <time dateTime={new Date(run.searchedAt).toISOString()}>{new Date(run.searchedAt).toLocaleString()}</time>. Locality matching does not verify a distance radius.</p>{(run.queries ?? [run.query]).map(query => <p key={query}>{query}</p>)}</details>}
